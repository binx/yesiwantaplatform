...
describe("parseRecipientsCsv", () => {
  ...
  it("reads what Excel exports: a byte-order mark, spaced and capitalised headers, first and last names", () => {
    ...
    expect(problems).toEqual([]);
    expect(recipients).toEqual([{ name: "Maya Okafor", line1: "12 Elm St", line2: null, city: "Marfa", state: "TX", postalCode: "79843" }]);
  ...
  it("imports the good rows and hands back the bad ones with what they held", () => {
    ...
    expect(problems).toEqual([
      { line: 3, message: expect.stringContaining("state"), draft: { name: "B", line1: "2 St", line2: null, city: "C", state: "California", postalCode: "90210" } },
    ]);
  });
  ...
  it("ignores a country column for now", () => {
    const { recipients, problems, ignored } = parseRecipientsCsv("name,street,city,state,zip,country\nA,1 St,B,CA,90210,US\n");
    expect(problems).toEqual([]);
