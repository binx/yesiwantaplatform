 ...
 * bundled fixture instead, which is useful for UI work without a database
 * running. Both paths are validated by the same schema.
 */
...
export async function loadStore(signal?: AbortSignal): Promise<Store> {
  if (import.meta.env.VITE_BELUGA_API !== "false") {
    ...
    // 503 means "no store yet", which the setup wizard handles.
