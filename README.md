...
**What's outstanding:** Phase 4 still owes `srcset` derivatives generated by `sharp` at upload, and the Lighthouse pass that confirms accessibility ≥ 95 — the structural work it measures (real `<img>` with alt text, labelled controls, keyboard-reachable buttons, scroll containers) has landed. v1's code stays in [`legacy/`](legacy/) as a reference; nothing builds from it.
