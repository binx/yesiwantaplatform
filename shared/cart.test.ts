...
describe("a cart line", () => {
  it("defaults the reply link on, and needs either recipients or a card to reply to — not both, not neither", () => {
    const plain = cartLineSchema.parse({ designs: [{ designId: "a", mailDate: "2026-09-14" }], recipients: [us] });
    expect(plain).toMatchObject({ replyLink: true, replyTo: null, replyToName: null });
