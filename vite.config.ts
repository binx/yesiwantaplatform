...
/*
 * Ports come from `.env`, which `npm run setup` writes, and which the API
 * reads through dotenv. Vite only exposes VITE_-prefixed variables to the
 * browser; it does *not* put `.env` into `process.env` for this file. So a
 * bare `process.env.API_PORT` was undefined whenever the shell had not set it,
 * the proxy fell back to :4000, and `dev:all` printed the API listening on the
 * `.env` port while every /api request got ECONNREFUSED. `loadEnv` with an
 * empty prefix reads the whole file; the shell still wins on conflict.
 */
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        // ASSETS_DIR. This used to bypass to Vite's own public/ server when a
        // file existed there, for bundled demo/ images nothing references any
        // more — and it raced the API's write of a freshly saved design
        // against Vite's public-directory watcher, serving the SPA shell in
        // place of the image on the first request. Always proxy.
